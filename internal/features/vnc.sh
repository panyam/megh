...
# the display stack + a terminal + X fonts to the box's LOCAL disk, starts it,
# and serves it on the tailnet. Idempotent.
#
# The display is blank until a GUI app draws on it. A terminal is auto-started;
# run other GUI apps against DISPLAY=:99 (e.g. Playwright headed after
# `megh enable playwright`), or use the fluxbox right-click menu.
set -uo pipefail
log() { echo "[megh-vnc] $*"; }

# 1. Ensure the display stack + terminal + fonts. apt is idempotent, so this is a
#    quick no-op when already installed (and picks up anything newly added).
log "ensuring display stack + xterm + fonts"
...
if ! { apt-get update -qq && apt-get install -y --no-install-recommends \
         ...
         xfonts-base fonts-dejavu-core; } >/tmp/enable-vnc-apt.log 2>&1; then
  log "apt install failed (see /tmp/enable-vnc-apt.log)"
  exit 1
fi

# 2. Start the display services (single instances).
...
# 3. A visible default: background colour + a font-correct terminal. Launched
#    each run (cheap) so a box with a broken/fontless xterm self-heals.
DISPLAY=:99 xsetroot -solid '#2e3440' >/dev/null 2>&1 || true
DISPLAY=:99 xterm -fa 'DejaVu Sans Mono' -fs 11 -geometry 120x34+40+40 >/tmp/xterm.log 2>&1 &
log "noVNC up on 127.0.0.1:6080 (DISPLAY=:99; a terminal is open, right-click for the fluxbox menu)"
